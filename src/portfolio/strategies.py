...
@dataclass
class RollingHRPStrategy:
    """
    Implements a rolling-window Hierarchical Risk Parity (HRP) strategy.

    Option A:
      - The hierarchy (clustering) is built from the window's return correlations.
      - Risk allocation between clusters uses the modular dependence/risk matrix Σ
        produced by `risk_estimator`.

    Workflow:
      1. Compute corr from `window_ret.corr()` for clustering.
      2. Estimate Σ from `risk_estimator.estimate(window_ret)` for cluster variances.
      3. Compute HRP weights via `hierarchical_risk_parity`.
      4. Store weights at monthly frequency.

    Parameters
    ----------
    risk_estimator : DependenceMatrixEstimator
        Object providing `.estimate(df_returns)` returning a risk matrix as DataFrame.
    start_year : int, default 2023
        First calendar year considered for rebalancing.
    lookback_years : int, default 1
        Length of the rolling estimation window.
    ...
    -------
    pd.DataFrame
        DataFrame of HRP weights indexed by rebalancing dates and with one column per asset.
    """
    ...
    def compute_weights(self, df_ret: pd.DataFrame) -> pd.DataFrame:
        ...
        for d in tqdm(rebal_dates):
            ...
            # Option A: corr from returns for clustering
            corr = window_ret.corr().to_numpy()

            # Sigma from your modular estimator for risk aggregation
            Sigma_df = self.risk_estimator.estimate(window_ret)
            Sigma = Sigma_df.to_numpy()
